...
import org.ohmyopensource.ohmyuniversity.canteen.security.GatewayAuthenticationFilter;
 ...
 *
 * The chat service sits behind the API gateway, which validates JWT tokens
 * and injects X-User-Id into every forwarded request. This service trusts
 * that header and does not perform JWT validation itself.
 *
 * Current auth model (development / no gateway):
 * - Pass X-User-Id header manually in Postman or curl to authenticate.
 * - Requests without X-User-Id are treated as anonymous and rejected on
 *   protected endpoints.
 *
